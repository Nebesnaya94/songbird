...
const trueBirdBlock = document.querySelector(".random-bird"); // блок для отображения угаданной птицы
const birdsBlocks = document.querySelector(".birds-container"); // блоки для выбора птиц
const birdItem = document.querySelectorAll(".bird-name"); // список имен птиц
const birdBtn = document.querySelectorAll(".birds-list-item"); // ячейки с именами птиц
const birdsList = document.querySelector(".birds-list"); // блок с именами птиц
const soundName = document.querySelector(".sound-name"); // название угаданной птицы (изначально ******)
const trueBirdImg = document.querySelector(".bird-image"); // картинка угаданной птицы
const birdIndicator = document.querySelectorAll(".li-ind"); // индикатор угадывания возле имени птицы
const pointsDisplay = document.querySelector(".score span"); // отображение баллов игры
const unseleсtedCard = document.querySelector(".birds-description-unselected"); // блок до выбора птицы
const seleсtedCard = document.querySelector(".birds-description-selected"); // блок с выбранной птицей
const selectedBirdImg = document.querySelector(".selected-image"); // картинка выбранной птицы
const selectedBirdNameRus = document.querySelector(".selected-name-rus"); // название выбранной птицы на русском
const selectedBirdNameLat = document.querySelector(".selected-name-lat"); // название выбранной птицы на латыни
const selectedBirdDescription = document.querySelector(".selected-description"); // описание выбранной птицы
const winSound = new Audio("../assets/sounds/true_select_sound.mp3"); // звук при верном выборе
const errSound = new Audio("../assets/sounds/false_select_sound.mp3"); // звук при неверном выборе

const winResultContainer = document.querySelector(".win-result-container"); // блок с результатом прохождения игры
const gameScore = document.querySelector(".win-score"); // счет результата игры в блоке выигрыша
const restartBtn = document.querySelector(".restart-btn"); // кнопка запуска новой игры

let audio = new Audio(); // обьект звука пения птиц
const birdSound = document.querySelector(".sound-music"); // блок с плеером
const time = document.querySelector(".time"); // изображение аудиодорожки
const btnPlay = document.querySelector(".play"); // кнопка запуска трека (пения птиц)
const durationTimeTracker = document.querySelector(".duration-time"); // общая длительность трека
const currentTimeTracker = document.querySelector(".current-time"); // текущая длительность трека

const levelPage = document.querySelectorAll(".page-item"); // отображение текущего уровня
levelPage[0].classList.add("active"); // подсветка первого уровня
const changeLevelBtn = document.querySelector(".change-level"); // кнопка переключения уровней

levelPage[0].classList.add("active"); // подсветка текущего уровня
changeLevelBtn.classList.add("disabled"); // изначально кнопка смены уровня неактивна

let lvl = 0; // начальный уровень
let points = 5; // начальный счет уровня игры

unseleсtedCard.classList.add("active"); // карточка с данными о птице до выбора

// нумерация списка названий птиц и уровней
for (let i = 0; i < birdsData[0].length; i++) {
  levelPage[i].setAttribute("level", i);
  birdItem[i].setAttribute("bird", i);
}

// начальное отображение при загрузке страницы
...
changeLevelBtn.addEventListener("click", () => {
  for (let i = 0; i < birdsData[lvl].length; i++) {
    birdBtn[i].removeEventListener("click", birdSelection);
  }
  if (lvl < birdsData.length - 1) {
    lvl++;
    levelPage[lvl - 1].classList.remove("active");
    newLevel(lvl);
  } else {
    gameScore.textContent = pointsDisplay.textContent;
    changeLevelBtn.classList.add("hidden");
    trueBirdBlock.classList.add("hidden");
    birdsBlocks.classList.add("hidden");
    winResultContainer.classList.add("active");
  }
});

// перезапуск игры по кнопке в блоке с результатом
restartBtn.addEventListener("click", () => {
  lvl = 0;
  pointsDisplay.textContent = "0";
  levelPage[5].classList.remove("active");
  changeLevelBtn.classList.remove("hidden");
  trueBirdBlock.classList.remove("hidden");
  birdsBlocks.classList.remove("hidden");
  winResultContainer.classList.remove("active");
  newLevel(lvl);
});

// запуск нового уровня игры
function newLevel(lvl) {
  points = 5;
  changeLevelBtn.classList.add("disabled");
  birdsList.classList.remove("disabled");
  levelPage[lvl].classList.add("active");
  gameBirds(lvl);
  soundPlayer(lvl);
  unseleсtedCard.classList.add("active");
  seleсtedCard.classList.remove("active");
  audio.pause();
  time.style.width = "0";
  btnPlay.classList.remove("active");
  soundName.textContent = "******";
  trueBirdImg.src = "../assets/images/bird_image.jpg";
  for (let i = 0; i < birdBtn.length; i++) {
    birdIndicator[i].style.background = "#444";
    birdBtn[i].classList.remove("active");
  }
}

// отображение данных птиц из обьекта и функция выбора птицы при нажатии на список
function gameBirds(lvl) {
  for (let i = 0; i < birdsData[lvl].length; i++) {
    birdItem[i].textContent = birdsData[lvl][i].name;
    birdBtn[i].addEventListener("click", birdSelection);
  }
}

// запуск трека по клику на кнопку плеера
btnPlay.addEventListener("click", function () {
  ...
  // изменение трекера времени каждую секунду
  setInterval(() => {
    let audioTime = Math.round(audio.currentTime);
    let audioLength = Math.round(audio.duration);
    time.style.width = (audioTime / audioLength) * 100 + "%";
    timeTracker(audio.currentTime, audio.duration);
...
// функция выбора птицы из списка
function birdSelection() {
  let n = document
    .getElementsByClassName("page-item active")[0]
    .getAttribute("level");

  let i = this.childNodes[2].getAttribute("bird");

  for (let x = 0; x < birdsData[n].length; x++) {
    birdBtn[x].classList.remove("active");
  }

  birdBtn[i].classList.add("active");
  unseleсtedCard.classList.remove("active");
  seleсtedCard.classList.add("active");
  selectedBirdImg.src = birdsData[n][i].image;
  selectedBirdNameRus.textContent = birdsData[n][i].name;
  selectedBirdNameLat.textContent = birdsData[n][i].species;
  selectedBirdDescription.textContent = birdsData[n][i].description;

  if (i == birdSound.getAttribute("num")) {
    soundName.textContent = birdsData[n][i].name;
    trueBirdImg.src = birdsData[n][i].image;
    pointsDisplay.textContent = +pointsDisplay.textContent + points;
    birdIndicator[i].style.background = "green";
    birdsList.classList.add("disabled");
    changeLevelBtn.classList.remove("disabled");
    audio.pause();
    btnPlay.classList.remove("active");
    winSound.pause();
    ...
    birdIndicator[i].style.background = "red";
    points--;
    errSound.pause();
...
//функция корректного отображения времени
...
// отображение текущего и общего времени в плеере
function timeTracker(current, duration) {
  let currentSec = Math.round(current);
  let currentMin = Math.floor(currentSec / 60);
  currentTimeTracker.textContent = `${isSingle(currentMin)}:${isSingle(
    currentSec - currentMin * 60
  )}`;

  let durationSec = Math.round(duration);
  let durationMin = Math.floor(durationSec / 60);
  durationTimeTracker.textContent = `${isSingle(durationMin)}:${isSingle(
    durationSec - durationMin * 60
  )}`;
}

// вычисление случайного индекса
function getRandomInd(max) {
  return Math.floor(Math.random() * max);
}

// функция выбора песни для плеера
function soundPlayer(lvl) {
  let ind = getRandomInd(6);
  birdSound.setAttribute("num", ind);
  audio.src = birdsData[lvl][ind].audio;
